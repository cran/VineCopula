...
BiCopLambda<-function(u1=NULL,u2=NULL,family="emp",par=0,par2=0,PLOT=TRUE,...)
{
	...
	if(family=="emp" || is.null(u1)==FALSE)	#empirical
	{
		n=length(v)
		nn<-length(u1)
		empLambda<-rep(0,n)
  ...
  if(PLOT)
  {
	if(family=="emp")	# only empirical one
	{
		if(min(empLambda[2:(nn-1)])<(-0.4))
			plot(v1,empLambda, type="l", ylab=expression(lambda(v)), xlab="v",ylim=c(-0.6,0), ...)
		else
			plot(v1,empLambda, type="l", ylab=expression(lambda(v)), xlab="v",ylim=c(-0.4,0), ...)

	}
	else if(family!="emp" && is.null(u1)==TRUE)	# only theoretical one
	{
		if(min(theoLambda[2:999])<(-0.4))
			plot(v1,theoLambda, type="l", ylab=expression(lambda(v)), xlab="v",ylim=c(-0.6,0), main=main, ...)
		else
			plot(v1,theoLambda, type="l", ylab=expression(lambda(v)), xlab="v",ylim=c(-0.4,0), main=main, ...)
...
gtLambda<-function(copula,param, len=1000)
{
	...
	else
	{
		if(copula==1)
		{
			...
			uu1 = runif(nn)
			vv2 = runif(nn)
			uu2 = .C("Hinv1",as.integer(1),as.integer(nn),as.double(vv2), as.double(uu1), as.double(rho),as.double(0), as.double(rep(0,nn)),PACKAGE='VineCopula')[[7]]
		...
		else if(copula==2)
		{
			...
			uu1 = runif(nn)
			vv2 = runif(nn)
			uu2 = .C("Hinv1",as.integer(2),as.integer(nn),as.double(vv2), as.double(uu1), as.double(rho),as.double(nu), as.double(rep(0,nn)),PACKAGE='VineCopula')[[7]]
