gof_PIT<-function(data,RVM,weight="Breymann",B=200, statisticName="AD", alpha=2)
{
	...
	else if(statisticName=="Anderson-Darling" || statisticName=="AD") statisticName=1
	...
	if(B==0)
	{
		...
		if(statisticName==1 && method==1)		# Macht nur Sinn bei Breymann
			...
			#stop("This statistic is not implemented or for this statistic no analytic form of the test statistic and p-value is available")
			pvalue=NULL
	}
	else
	{
