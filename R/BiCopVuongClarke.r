...
vuong.clarke.sim<-function(xy,cop.model1,cop.model2,correction,level)
{
  ...
  loglik.model2<-data.frame(sapply(cop.model2,function(x){my.loglik(fam=x,data=xy)},USE.NAMES=TRUE))
  ...
  for(i in 1:length(cop.model2))
  {
    ...
    {
      if(cop.model1 %in% c(2,7:10,17:20,27:30,37:40,104,114,124,134,204,214,224,234)){ p1<-2}
      if(cop.model2[i] %in% c(2,7:10,17:20,27:30,37:40,104,114,124,134,204,214,224,234)){p2<-2}
  
      vuong.result.temp[[i]]<-vuong.test(loglik.model1,loglik.model2[,i],alpha=level,p1=p1,p2=p2,print.result=FALSE,correction)
      names(vuong.result.temp)[i]<-cop.model2[i]
      clarke.result.temp[[i]]<-clarke.test(loglik.model1,loglik.model2[,i],alpha=level,p1=p1,p2=p2,print.result=FALSE,correction)
      names(clarke.result.temp)[i]<-cop.model2[i]
    }
  }
...
for(i in 1:T)
{
	out <- .C("LL_mod",
	      ...
	      as.double(0),
              PACKAGE='VineCopula')
	loglik.temp[i] <- out[[7]]
	if (loglik.temp[i] %in% c(NA,NaN,Inf)) loglik.temp[i] <- 1e10
}
...
{ 
  ...
  if(correction == FALSE) m.i<-loglik.model1-loglik.model2
  else if(correction == "Akaike") m.i<-loglik.model1-loglik.model2-(p1-p2)/n
  else if(correction == "Schwarz") m.i<-loglik.model1-loglik.model2-(p1-p2)*log(n)/(2*n)
...
score<-function(dat)
{
  ...
  {
    yes<-sum(dat==1,na.rm=TRUE)       # z?hle wieviel mal kommt yes vor
    no<-sum(dat==2,na.rm=TRUE)*(-1)   # z?hle wieviel mal kommt no vor
    score.out<-yes+no
  }
  if(sum(is.na(dat))==length(dat))
  {
...
BiCopVuongClarke<-function(u1,u2,familyset=NA,correction=FALSE,level=0.05)
{
  ...
  if(is.na(familyset[1])) familyset = c(1:10,13,14,16:20,23,24,26:30,33,34,36:40,41,51,61,71,104,114,124,134,204,214,224,234)
# Sicherheitsabfragen
...
for(i in 1:length(familyset))
	{
	if(!(familyset[i] %in% c(1:10,13,14,16:20,23,24,26:30,33,34,36:40,41,51,61,71,104,114,124,134,204,214,224,234))) stop("Copula family not implemented.")
	}
