	...
	for(i in (n-1):1){
      ...
      Params2[k,i] = outcop$par2
        ...
        #V$direct[k-1,i,] = outcop$CondOn.2
				V$direct[k-1,i,] = .C("Hfunc1", as.integer(Types[k,i]), as.integer(N),
                              as.double(zr1), as.double(zr2), 
                              as.double(Params[k,i]), as.double(Params2[k,i]),
                              as.double(rep(0,N)), PACKAGE='VineCopula')[[7]]
			if(CondDistr$indirect[k-1,i])
        #V$indirect[k-1,i,] = outcop$CondOn.1
				V$indirect[k-1,i,] = .C("Hfunc2", as.integer(Types[k,i]), as.integer(N),
                                as.double(zr2), as.double(zr1),
                                as.double(Params[k,i]), as.double(Params2[k,i]),
                                as.double(rep(0,N)), PACKAGE='VineCopula')[[7]]
		}
	}
