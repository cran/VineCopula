RVineTreePlot = function(data=NULL, RVM, method="mle", max.df=30, max.BB=list(BB1=c(5,6),BB6=c(6,6),BB7=c(5,6),BB8=c(6,1)), tree="ALL", edge.labels=c("family"), P=NULL){
  ...
  if(is.null(RVM$names)) RVM$names = paste("V",1:d,sep="")
  
  empTauMat = matrix(NA,d,d)

  if(!is.null(data)){
  
    seqpar = RVineSeqEstTau(data,RVM, method=method, se=FALSE, max.df=max.df, max.BB=max.BB, progress=FALSE)
  
    RVM$par = seqpar$par
    RVM$par2 = seqpar$par2
    
    if(any(edge.labels=="emptau")) empTauMat = round(seqpar$tau,2)
  
  }
  
  theoTauMat = round(RVinePar2Tau(RVM),2)
  if(any(edge.labels=="par")) parMat = round(RVM$par,2)
  if(any(edge.labels=="par2")) parMat2 = round(RVM$par2,2)
  ...
  if(tree != "ALL" && tree > d-1) stop("Selected tree does not exist.")

  if(tree == "ALL") tree = 1:(d-1)
  ...
  if(edge.labels[1]!=FALSE){
    for(jj in 1:numlabels){
      if(edge.labels[jj] == "family") elabels[[1]][1,jj] = BiCopName(RVM$family[d,d-1],short=TRUE)
      if(edge.labels[jj] == "par") elabels[[1]][1,jj] = parMat[d,d-1]
  ...
  for(i in (d-2):1){
    ...
    if(edge.labels[1]!=FALSE){
      for(jj in 1:numlabels){
        if(edge.labels[jj] == "family") elabels[[1]][d-i,jj] = BiCopName(RVM$family[d,i],short=TRUE)
        if(edge.labels[jj] == "par") elabels[[1]][d-i,jj] = parMat[d,i]
    ...
    for(k in 1:(d-i-1)){
      ...
      if(length(M[(d-k):d,i])>=3){
        if(setequal(M[(d-k):d,i],ee_old)){
          ...
          for(j in 1:(d-i-k)){
            if(setequal(M[(d-k):d,i],edges[[k+1]][j,1,])) edges[[k+1]][d-i-k,2,] = edges[[k+1]][j,1,]
            if(setequal(M[(d-k):d,i],edges[[k+1]][j,2,])) edges[[k+1]][d-i-k,2,] = edges[[k+1]][j,2,]
          }
        }
      }else{
      ...
      if(edge.labels[1]!=FALSE){
        for(jj in 1:numlabels){
          if(edge.labels[jj] == "family") elabels[[k+1]][d-i-k,jj] = BiCopName(RVM$family[d-k,i],short=TRUE)
          if(edge.labels[jj] == "par") elabels[[k+1]][d-i-k,jj] = parMat[d-k,i]
      ...
      # identify conditioned and conditioning sets
      ee = c(sort(c(setdiff(ee,M[(d-k):d,i]),setdiff(M[(d-k):d,i],ee))),sort(intersect(ee,M[(d-k):d,i])))
    }
  ...
  for(i in 1:(d-1)){
    ...
    if(edge.labels[1]!=FALSE) E(gg[[i]])$name = elabels2[[i]]
  }
...
RVineSeqEstTau<-function(data,RVM, method="mle", se=FALSE, max.df=30, max.BB=list(BB1=c(5,6),BB6=c(6,6),BB7=c(5,6),BB8=c(6,1)), progress=FALSE)
{
	data=as.matrix(data)
	n = dim(RVM)
	N = nrow(data)
...
if(se==FALSE)
	return(list(par=Params,par2=Params2,tau=empTauMat))
else
	return(list(par=Params,par2=Params2,tau=empTauMat, se=seMat1, se2=seMat2))
}
