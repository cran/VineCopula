...
cop.pdf <- function(u1, u2, param, copula)
{
  ...
  t2 = qnorm(p=u2)
  rho=param
  return( 1.0/sqrt(1.0-rho^2)*exp(-(rho^2*(t1^2+t2^2)-2.0*rho*t1*t2)/(2.0*(1.0-rho^2))) )    
  }
  else if(copula==2)	# t-copula
  {
  rho=param[1]
  nu=param[2]
  ...
  t1 = qt(u1,nu)
  t2 = qt(u2,nu)
  return( 1.0/(2*pi*sqrt(1.0-rho^2)*dt(t1,nu)*dt(t2,nu)) * (1.0+(t1^2+t2^2-2.0*rho*t1*t2)/(nu*(1.0-rho^2)))^(-(nu+2)/2) );     
  }
  else if(copula==3)	#Clayton
  {
  theta=param
  return((1.0+theta)*(u1*u2)^(-1.0-theta)*(u1^(-theta)+u2^(-theta)-1.0)^(-2.0-1.0/theta))
  }
  ...
  t2 = exp(-t1^(1.0/theta));
  return( t2/(u1*u2)*t1^(-2.0+2.0/theta)*(log(u1)*log(u2))^(theta-1.0)*(1.0+(theta-1.0)*t1^(-1.0/theta)) )
  }
  else if(copula==5)	#Frank
  {
  theta=param
  return ( (theta*(exp(theta)-1.0)*exp(theta*u2+theta*u1+theta))/(exp(theta*u2+theta*u1)-exp(theta*u2+theta)-exp(theta*u1+theta)+exp(theta))^2.0 )
  }
...
{
if(margins=="norm")
  return(cop.pdf(u1=pnorm(x1), u2=pnorm(x2), param=param, copula=copula) * dnorm(x1) * dnorm(x2))
else if(margins=="t")
  return(cop.pdf(u1=pt(x1, df=margins.par), u2=pt(x2, df=margins.par), param=param, copula=copula) * dt(x1, df=margins.par) * dt(x2, df=margins.par))
else if(margins=="unif")
  return(cop.pdf(u1=x1, u2=x2, param=param, copula=copula) )
else if(margins=="gamma")
  return(cop.pdf(u1=pgamma(x1, shape=margins.par[1], scale=margins.par[2]), u2=pgamma(x2, shape=margins.par[1], scale=margins.par[2]), param=param, copula=copula) * dgamma(x1, shape=margins.par[1], scale=margins.par[2]) * dgamma(x2, shape=margins.par[1], scale=margins.par[2]))
else if(margins=="exp")
  return(cop.pdf(u1=pexp(x1, rate=margins.par), u2=pexp(x2, rate=margins.par), param=param, copula=copula) * dexp(x1, rate=margins.par) * dexp(x2, rate=margins.par))
}
...
family="emp", par=0, par2=0, PLOT=TRUE, margins="norm", margins.par=0, xylim=NA, ...)
{
  ...
  if(margins!="norm" && margins!="t" && margins!="exp" && margins!="gamma" && margins!="unif") stop("The function only supports Gaussian ('norm'), Student t ('t'), exponential ('exp'), Gamma ('gamma') and uniform ('unif') margins.")

  if(margins=="t" && margins.par<=0) stop("The degrees of freedom parameter for the Student t margins has to positive.")
  if(margins=="Gamma" && length(margins.par)!=2) stop("For Gamma margins two parameters are required in 'margins.par'.")
  if(margins=="exp" && margins.par==0) stop("Exponential margins require one parameter in 'margins.par'.")
  if(margins=="unif" && family==0) stop("The combination independence copula and uniform margins is not possible because all z-values are equal.")
  ...
  if(!is.na(xylim) && length(xylim)!=2) stop("'xylim' has to be a vector of length 2.")
