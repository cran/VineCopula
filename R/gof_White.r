...
gof_White2<-function(data,RVM,B=200)
{
	...
	for(t2 in 1:T)
	{
		...
		for(i in 1:(dd+tt))		# laufe ?ber alle Parameter
		{
			if(i<=dd)
			{
				par_plus=RVM$par
				par_plus[i]=par_plus[i]+0.00001		# + epsilon
				par2_plus=RVM$par2
				par_minus=RVM$par
				par_minus[i]=par_minus[i]-0.00001		# - epsilon
				par2_minus=RVM$par2
			}
			else
			{
				par_plus=RVM$par
				par2_plus=RVM$par2
				par2_plus[i]=par2_plus[i]+0.00001		# + epsilon
				par_minus=RVM$par
				par2_minus=RVM$par2
				par2_minus[i]=par2_minus[i]-0.00001		# - epsilon
			}
			RVM_plus=RVineMatrix(Matrix=RVM$Matrix,family=RVM$family,par=matrix(par_plus,d,d),par2=matrix(par2_plus,d,d))
			RVM_minus=RVineMatrix(Matrix=RVM$Matrix,family=RVM$family,par=matrix(par_minus,d,d),par2=matrix(par2_minus,d,d))
			...
			gradD[,i]=gradD[,i]+(Dplus-Dminus)/2/0.00001		# finite Differenzen
		}
